...
/**
 * FiltersGrid (SurfSpots)
 * ----------------------
 * - uniqueSpots: [{label, value}] where value = spot.slug
 * - uniqueZone:  [{label, value}] where value = surfzone_slug
 */
...
export default function FiltersGrid({
  ...
  const sortedZones = [...uniqueZone].sort((a, b) =>
    (a.label || "").localeCompare(b.label || ""),
  );
